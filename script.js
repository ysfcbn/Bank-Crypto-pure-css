...
const setLocalStrogeFunc = function () {
	...
	// localStorage.setItem("transferDates", JSON.stringify(transferDate));
...
const getLocalStroge = function () {
	...
	// if (!depositDates) return;
	// depositDate = depositDates;
	// depositDate = depositDate.map((date) => new Date(date));
	// console.log(depositDate);

	// if (!transferDates) return;
	// transferDate = transferDates;
	// transferDate = transferDate.map((date) => new Date(date));
...
transferMoneyBtn.addEventListener("click", function () {
	...
	if (transferMoneyAmount.value > 0 && transferMoneyAmount.value !== "") {
		...
		countArr = [];
		allUsersAccountMov[currentUser[0].userID].transferDate.push(dateTransfer);
		console.log(allUsersAccountMov[currentUser[0].userID].transferDate);
